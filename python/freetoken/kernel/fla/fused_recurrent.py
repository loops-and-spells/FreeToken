...
from freetoken.kernel.fla.op import exp
...
def fused_recurrent_kda_packed_decode(
    ...
    # NOTE: upstream sglang routes large batches through a CUDA kda_packed_decode
    # extension here; FreeToken doesn't carry that extension, so the triton kernel
    # below serves every batch size.
    kda_decode_cuda = None
    if use_qk_l2norm_in_kernel and kda_decode_cuda is not None:
